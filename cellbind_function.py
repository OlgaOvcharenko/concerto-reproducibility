...
def create_single_cell_network_teacher(mult_feature_name: str, tf_path: str, super_parameters=None):
...
def create_single_cell_network_student(mult_feature_name: str, tf_path: str, super_parameters=None):
    if super_parameters is None:
        super_parameters = {'batch_size12': 32,
                            'batch_size13': 32,
                            'epoch_pretrain': 3,
            ...
            while opit1a.has_value() or opit2.has_value():
                ...
                with tf.GradientTape() as tape:
                    if super_parameters["combine_omics"]:
                        ...
                        loss = clip_loss(z1a, z2, temperature) + clip_loss(z1b, z3, temperature)
...
def cellbind_train_multimodal_ts(mod1a_tf_path: str, mod2_tf_path: str, mod1b_tf_path: str, mod3_tf_path: str, weight_path: str, mod1_network_teacher, mod2_network_teacher, mod3_network_teacher, mod1_network_student, mod2_network_student, mod3_network_student, super_parameters=None):
    train_log_dir = 'logs_tensorboard/gradient_tape_cell_bind/' + f'{super_parameters["model_type"]}_multi_{super_parameters["data"]}_{super_parameters["batch_size12"]}_{super_parameters["batch_size13"]}_{super_parameters["epoch_pretrain"]}_{super_parameters["lr"]}_{super_parameters["drop_rate"]}_{super_parameters["attention_s"]}_{super_parameters["attention_t"]}_{super_parameters["heads"]}' + '/train'
    train_summary_writer = tf.summary.create_file_writer(train_log_dir)
    ...
    for epoch in range(super_parameters['epoch_pretrain']):
        for mod1a_file, mod2_file, mod1b_file, mod3_file in itertools.zip_longest(train_source_list_mod1a, train_source_list_mod2, train_source_list_mod1b, train_source_list_mod3):
            ...
            while opit1a.has_value() or opit2.has_value():
                ...
                with tf.GradientTape() as tape:
                    if super_parameters["combine_omics"]:
                        # Teachers
                        z1a = mod1_network_teacher([[source_features_mod1a], [source_values_mod1a]], training=True)
                        z2 = mod2_network_teacher([[source_features_mod2], [source_values_mod2]], training=True)
                        
                        z1b = mod1_network_teacher([[source_features_mod1b], [source_values_mod1b]], training=True)
                        z3 = mod3_network_teacher([[source_features_mod3], [source_values_mod3]], training=True)
                        
                        # Students
                        z1a_s = mod1_network_student([source_values_mod1a], training=True)
                        z2_s = mod2_network_student([source_values_mod2], training=True)
                        
                        z1b_s = mod1_network_student([source_values_mod1b], training=True)
                        z3_s = mod3_network_student([source_values_mod3], training=True)
                        
                        if super_parameters["model_type"] == 2:
                            loss = clip_loss(z1a, z2, temperature) + clip_loss(z1b, z3, temperature) \
                                + clip_loss(z1a_s, z2_s, temperature) + clip_loss(z1b_s, z3_s, temperature) \
                                + clip_loss(z1a, z2_s, temperature) + clip_loss(z1b, z3_s, temperature) \
                                + clip_loss(z1a_s, z2, temperature) + clip_loss(z1b_s, z3, temperature) \
                ...
                variables = [mod1_network_teacher.trainable_variables, mod2_network_teacher.trainable_variables, mod3_network_teacher.trainable_variables, mod1_network_student.trainable_variables, mod2_network_student.trainable_variables, mod3_network_student.trainable_variables, [temperature]]
        ...
        mod1_network_teacher.save_weights(
            weight_path + f'encoder_GEX_weight_{super_parameters["data"]}_{super_parameters["batch_size12"]}_{super_parameters["batch_size13"]}_model_{super_parameters["combine_omics"]}_{super_parameters["model_type"]}_epoch_{epoch+1}_{super_parameters["lr"]}_{super_parameters["drop_rate"]}_{super_parameters["attention_t"]}_{super_parameters["attention_s"]}_{super_parameters["heads"]}.h5')
        mod2_network_teacher.save_weights(
            weight_path + f'encoder_ADT_weight_{super_parameters["data"]}_{super_parameters["batch_size12"]}_{super_parameters["batch_size13"]}_model_{super_parameters["combine_omics"]}_{super_parameters["model_type"]}_epoch_{epoch+1}_{super_parameters["lr"]}_{super_parameters["drop_rate"]}_{super_parameters["attention_t"]}_{super_parameters["attention_s"]}_{super_parameters["heads"]}.h5')
        mod2_network_teacher.save_weights(
            weight_path + f'encoder_ATAC_weight_{super_parameters["data"]}_{super_parameters["batch_size12"]}_{super_parameters["batch_size13"]}_model_{super_parameters["combine_omics"]}_{super_parameters["model_type"]}_epoch_{epoch+1}_{super_parameters["lr"]}_{super_parameters["drop_rate"]}_{super_parameters["attention_t"]}_{super_parameters["attention_s"]}_{super_parameters["heads"]}.h5')
