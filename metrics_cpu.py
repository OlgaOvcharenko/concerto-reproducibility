...
df.to_csv(f'./Multimodal_pretraining/plots/metrics/{data.split("/")[-1][:-5]}_metrics_False.csv', min_max_scale=False)

df = bm.get_results(min_max_scale=False)
print(df)
df = df.round(decimals=3)
df.to_csv(f'./Multimodal_pretraining/plots/metrics/{data.split("/")[-1][:-5]}_metrics_True.csv', min_max_scale=True)
...
bm.plot_results_table(save_dir='./Multimodal_pretraining/plots/metrics/', min_max_scale=False)
