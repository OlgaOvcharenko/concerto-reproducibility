...
# Switch things up. Error has not been found yet but want to continue.
#adata = adata[adata.obs['batchlb']=='Batch1'][:7378].concatenate(adata[adata.obs['batchlb']=='Batch2'][:7378])
print(adata)
...
for epoch in [100, 200, 300, 400]:
    saved_weight_path = save_path + f'weight/weight_encoder_epoch{epoch}.h5' #You can choose a trained weight or use None to default to the weight of the last epoch.
    ...
    np.save(f"{save_path}/embeddings/embedding{epoch}", embedding)
