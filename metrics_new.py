...
only_files = [f for f in listdir(f'./Multimodal_pretraining/data/{data}/') if isfile(join(f'./Multimodal_pretraining/data/{data}/', f)) if f.startswith(f'Multimodal_pretraining/data/{data}/{data}_bc_')]
print(only_files)
exit()


path = filename = f'./Multimodal_pretraining/data/{data}/{data}_bc_{combine_omics}_mt_{model_type}_bs_{batch_size}_{epoch}_{lr}_{drop_rate}_0_1_{heads}_{repeat}.h5ad'
